...
def with_exponential_backoff(attempts: int = 3, base_delay: float = 0.5):
    """Retry decorator with logarithmic wait scaling.
    
    Implements progressive delay between parse failures to
    allow transient API issues to resolve.
    
    Parameters:
        attempts: Maximum execution attempts
        base_delay: Starting wait interval (seconds)
        
    Returns:
        Enhanced function with built-in fault tolerance
    """
    import logging
                ...
                try:
                    ...
                    if attempt < attempts:
                        retry_logger.warning(
                            f"Parse attempt {attempt}/{attempts} failed: {exc}. "
                        ...
                        retry_logger.error(
                            f"All {attempts} parse attempts exhausted. Last error: {exc}"
                        )
            return []
        return wrapper
    return decorator
...
class AttackStrategyGenerator:
    """
    AI-powered security test plan generator using multiple LLM providers.
    """
    
    def __init__(self, model_provider: str = "openai", model_name: str = None, debug: bool = False):
        """Configure attack strategy generation engine.
        
        Parameters:
            model_provider: AI backend (openai/anthropic/gemini/ollama/litellm)
            model_name: Override default model identifier
            debug: Activate verbose logging
        """
        self.model_provider = model_provider.lower()
        self.debug = debug
        
        # Initialize LiteLLM if provider is litellm
    ...
    def _set_system_prompts(self):
        ...
        if self.model_provider in ["openai", "ollama", "litellm"]:
            self.system_prompt = self.openai_system_prompt
        elif self.model_provider == "anthropic":
            self.system_prompt = self.anthropic_system_prompt
        elif self.model_provider == "gemini":
            # Remove triple backticks for Gemini
            self.system_prompt = self.openai_system_prompt.replace("```yaml", "").replace("```", "")
        else:
            raise ValueError(f"Unsupported model provider: {self.model_provider}")

    @with_exponential_backoff(attempts=3, base_delay=0.5)
    def plan(self, message: str, reasoning: str = "medium") -> List[Dict]:
        ...
        if self.model_provider == "openai":
            ...
            return self._gemini_plan(message)
        elif self.model_provider == "litellm":
            return self._litellm_plan(message, reasoning)
        else:
    ...
    def _gemini_plan(self, message: str) -> List[Dict]:
        prompt = f"{self.system_prompt}\n\n{message}"
        
        try:
            # Use new google.genai API with safety settings disabled
            ...
            if not content:
                if self.debug:
                    print(f"[Debug] Gemini response structure: {type(response)}")
            ...
            try:
                ...
                # Return fallback plan if YAML parsing fails
                return [{
                    'title': 'Basic Security Check',
                    'description': 'Unable to parse LLM output. Using fallback security assessment plan.',
                    'priority': 'medium'
                },
                {
                    'title': 'General Vulnerability Testing',
                    'description': 'Test for common web vulnerabilities including injection, XSS, and authentication bypass.',
                    'priority': 'high'
                }]
            ...
            return [{
                ...
                'description': 'API error occurred. Conducting basic security assessment with fallback methods.',
                'priority': 'high'
            },
            {
                'title': 'Manual Security Testing',
                'description': 'Perform manual security testing focusing on common attack vectors.',
                'priority': 'medium'
            }]
    
    def _litellm_plan(self, message: str, reasoning: str = "medium") -> List[Dict]:
        """LiteLLM-specific implementation of planning."""
        headers = {
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 401:
                    ...
                    print(error_msg)
                    if self.debug:
                        print(f"API Base: {self.litellm_api_base}")
                ...
                yaml_str = result["choices"][0]["message"]["content"]
                ...
                if self.debug:
                    print(f"LiteLLM request error on attempt {attempt + 1}: {e}")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    retry_delay *= 2
                else:
